...
function getFormData(formSubmitObj, translationConfig) {
  ...
  for (var propertyName in translationConfig) {
    ...
    if (translationInfo.hasOwnProperty('answers') &&
      ...
      var typeTranslations = translationInfo.typeTranslations;
      if (typeof typeTranslations !== "undefined") {
        Object.keys(typeTranslations).forEach((key) => {
          if (answerInForm.startsWith(typeTranslations[key])) {
            answerInForm = key;
          }
        });
      }
